package main

import (
	"fmt"
	"strconv"
	"sync"
)

// Go语言中内置的map不是并发安全的。
// 即Go语言中的map是不支持被多个goroutine并发运行的

/* 这种场景下就需要为map加锁来保证并发的安全性了
Go语言的sync包中提供了一个开箱即用的并发安全版map，即sync.Map,
开箱即用表示不用像内置的map一样使用make函数初始化就能直接使用。
同时sync.Map内置了诸如Store、Load、LoadOrStore、Delete、Range等操作方法。*/
//sync.Map里的几个操作：
/*
Store:设置值
Load:取值
LoadorStore:先加载key,执行Load;如果没有key,则执行Store
Delete:删除
Range:遍历
*/
//...............................................................
//...............................................................
/*
var m = make(map[string]int)//声明m为普通版map
var lock sync.Mutex
func get(key string) int {
	return m[key]
}
func set(key string, value int) {
	m[key] = value
}
func main() {
	wg := sync.WaitGroup{}
	//上面的代码开启少量几个goroutine的时候可能没什么问题，
	//当并发多了之后执行上面的代码就会报fatal error: concurrent map writes错误。
	for i := 0; i < 200; i++ {
		wg.Add(1)
		go func(n int) {
			lock.Lock() //设置锁来保证Map并发的安全性
			key := strconv.Itoa(n)
			set(key, n)
			lock.Unlock()
			fmt.Printf("k=:%v,v:=%v\n", key, get(key))
			wg.Done()
		}(i)
	}
	wg.Wait()
}
*/
//..............................................................
//..............................................................

//使用sync.Map
var m = sync.Map{} //声明m为并发安全版的Map

func main() {
	wg := sync.WaitGroup{}
	for i := 0; i < 20; i++ {
		wg.Add(1)
		go func(n int) {
			key := strconv.Itoa(n)
			m.Store(key, n)         //必须使用sync.Map内置的Store方法设置键值对
			value, _ := m.Load(key) //必须使用sync.Map内置的Load方法根据key取值
			fmt.Printf("k=:%v,v:=%v\n", key, value)
			wg.Done()
		}(i) //这里使用i当参数传进n
	}
	wg.Wait()
}
